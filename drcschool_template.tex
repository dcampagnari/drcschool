...
% Die Klassenoption `hyperworksheet' lädt hyperref und definiert ein ausfüllbares Arbeitsblatt.
...
\newcommand{\code}[1]{%                                                         %
   ...
   \def\\{\textbackslash}\ignorespaces#1\par                                    %
...
Es gibt zwei \emph{eigene} Optionen: \opt{nofonts} und \opt{hyperworksheet}.
Die zweite Option beschreibe ich am Ende dieses Dokumentes
(s.~Abschnitt~\ref{sec:hy}). Die erste hebt die Standardeinstellung für die
Schriftart aus. Standardmäßig wird eine serifenlose Schrift gewählt:\footnote{%
   Für die Neugierigen: \pkg{tgheros} mit \pkg{newtxsf}, plus einige persönlichen Vorlieben
   (ich mag die Ziffern von \pkg{newtxsf} nicht).}
...
Soll jemand meine Standardeinstellung nicht mögen,\footnote{%
   ,,\texttt{drc}`` ist eine Abkürzung für meinen Namen (,,\texttt{r}`` steht für meinen
   zweiten Vornamen, nicht für den Doktortitel). Also ja, ich gestalte die
   Klasse so, dass sie \emph{mir} gefällt. Dass andere sie nützlich finden, ist nur ein
   netter Nebeneffekt~\texttt{;-)}}
...
\begin{compactdesc}
\item[Schrift- und Sprachpaketen] \pkg{fontenc} mit Option \opt{T1}, \pkg{microtype}, \pkg{babel} mit Option \opt{ngerman}.
...
\item[Verschiedenes] \pkg{amsgen}, \pkg{environ}\footnote{%
 ...
 \pkg{paralist}, \pkg{icomma}, \pkg{tikz}, \pkg{pgfplots}.
\end{compactdesc}
...
Dokument mit Lua\TeX\ oder X\lower.5ex\hbox{\reflectbox{E}}\kern-.16em\TeX\
...
Im obigen Beispiel wurde die Umgebung mit dem optionalen Argumenten \opt{1} gestartet.
...
obligatorischen Argumenten muss eine Reihe von verschiedenen Deklarationen der Form\code{
...
\begin{worksheet}[Horizontale Auflistungen]
...
\exercise[Kurzfragen wie im Lambacher-Schweizer]
...
\begin{worksheet}[Schwierigkeitssymbole und Wahr-Falsch-Tabellen]

\hard\exercise
Schwere Aufgaben können mit \cs{hard}\cs{exercise} erzeugt werden. Die Schwierigkeitssymbole
sind denen aus dem Lambacher-Schweizer nachempfunden.

\medium\exercise[Bla]
Mittelschwere Aufgaben kommen aus \cs{medium}\cs{exercise[Bla]}.

Zur Verfügung stehen auch die Symbole \cs{calculator} \calculator\
und \cs{nocalculator} \nocalculator. Diese nehmen \TikZ-Optionen als
optionales Argument:
\begin{tabular}[t]{l@{\space\textrightarrow\space}l}
...
Bitte beachten: Die drei Makros \cs{hard}, \cs{medium}
und \cs{easy} dürfen \emph{nur} vor dem Befehl \cs{exercise}
verwendet werden. Der Code\code{
   \\hard\\SomeOtherMacro
}
wird einen Fehler erzeugen.
Will man die Symbole irgendwo haben, so gibt es dafür die Befehle
\cs{easysymbol} (\easysymbol), \cs{mediumsymbol} (\mediumsymbol), und \cs{hardsymbol} (\hardsymbol).
...
\exercise[Arbeitsblatt ohne Lösung]
Es gibt auch eine Umgebung \env{worksheet*}, für die \emph{keine} Lösung angegeben
wird. Ich verwende sie z.B.~wenn ich einfach einen längeren Text austeilen will.
...
werden. \IfSolutionT{(Zum Beispiel hier.)\space\ignorespaces} In der Tat, ist es
...
\begin{solution}
Es gibt mehrere Auswege. Man kann natürlich so was schreiben wie:\code{
   \\answer (Siehe Text.)
}
...
\cluecard{Vorne 2}{Hinten 2\\Standardmäßig sind 2x3 Kärtchen auf einem Blatt.}
...
Die Klasse definiert eine Umgebung \env{colorminipage}, die sich wie \env{minipage} verhält,
aber ihren Inhalt in einer farbigen Box setzt. Es handelt sich im Grunde um eine Kombination
aus \cs{colorbox} und \env{minipage}, aber die Textbreite wird so gerechnet, dass die angegebene
Breite auch den Rand berücksichtigt. Die Randgröße ist ganz normal der Wert von \cs{fboxsep}.
Alle optionalen Argumente zu \env{minipage} können wie
...
kriegt man:
\rlap{\tikz{\draw[->](0,0)--(2cm,3ex);}}
\drcput(2cm,3ex){\textcolor{red}{X}}
\drcput[c](2cm,3ex){\textcolor{blue}{X}}
\drcput[l](2cm,3ex){\textcolor{green}{X}}.
...
Als ältere Variante existiert noch das Makro\code{
...
Ähnlich zu \cs{wrap} existiert auch\\[.5ex]
\hspace*{2em}\cs{wraptikz[\param{\TikZ\ Optionen}][\param{\opt{l} oder \opt{r}}]\{\param{\TikZ\ Code}\}}\\[.5ex]
