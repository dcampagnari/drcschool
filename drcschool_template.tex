...
\onlytikz\ markiert. Features, die auch ohne \TikZ\ funktionieren aber leicht
anders aussehende Ergebnisse liefern, werden mit \withtikz\ markiert.
\end{description}
...
mit folgenden Keys:
\begin{compactdesc}
\item[\opt{extrarowheight=\param{Länge}}] setzt \cs{extrarowheight}
\item[\opt{arraystretch=\param{Zahl}}] setzt \cs{arraystretch}
\item[\opt{tabcolsep=\param{Länge}}] setzt \cs{tabcolsep}
...
\end{compactdesc}
Die letzten Zeile muss leicht anders betrachtet werden. In der Tat, mit der Option \opt{extrarowheight}
...
\begin{worksheet}[\texorpdfstring{\leavevmode\llap{\withtikz\quad}}{}Schwierigkeitssymbole und sonstige Teilaufgaben]
...
\deadly\exercise
\ldots\ auch \cs{deadly}\cs{exercise} \texttt{;-)} Falls \TikZ\ nicht verwendet
wird, stehen die Symbole noch zur Verfügung aber sehen etwas anders aus (sie
werden dann aus \pkg{fontawesome5} genommen).
...
\begin{solution}
Bitte beachten: Die Makros \cs{easy}, \ldots, \cs{hardest} dürfen \emph{nur}
...
Ähnlich zu \cs{wrap} existiert auch\rlap{\quad\onlytikz}
\bcode
